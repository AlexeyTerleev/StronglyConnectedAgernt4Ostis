...
#include "IsStronglyConnectedAgent.hpp"
...
namespace stronglyConnectedModule
{
    ...
    ScAddr get_set_transposed_arcs(const std::unique_ptr<ScMemoryContext>& context, ScAddr graph){

        ScAddr transposed_rrel_arcs = context->CreateNode(ScType::Const);

        ScAddr arcs_set = get_set_arcs(context, graph);
        ...
        while (arcs_it->Next()){
            ScAddr transposed_arc =
                    context->CreateEdge(ScType::EdgeAccessConstPosPerm,
                                        context->GetEdgeTarget(arcs_it->Get(2)),
                                        context->GetEdgeSource(arcs_it->Get(2)));
            context->CreateEdge(ScType::EdgeAccessConstPosPerm, transposed_rrel_arcs, transposed_arc);
        }
        return transposed_rrel_arcs;
    }
    ...
    void check_graph(const std::unique_ptr<ScMemoryContext>& context, ScAddr graph){
        ...
        context->HelperSetSystemIdtf("strongly connected graph", strongly_connected_set);

        SC_LOG_INFO("~~~~~~~~" + context->HelperGetSystemIdtf(graph) + "~~~~~~~~");

        if(do_dfs(context, graph, get_set_arcs(context, graph)) &&
            do_dfs(context, graph, get_set_transposed_arcs(context, graph)))
        {
    ...
    SC_AGENT_IMPLEMENTATION(IsStronglyConnectedAgent)
    {
        SC_LOG_INFO("IsStronglyConnectedAgent was activated");
